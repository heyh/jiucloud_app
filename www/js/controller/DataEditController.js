 ...
 */
angular.module('dataEdit.controllers', ['DataEdit.services'])
    .controller('DataEditCtrl', function (DataEditService, DataDetailsService, DataAddService, $scope, $state,  $ionicLoading, $stateParams, $ionicModal, $ionicViewSwitcher, localStorage, $stateParams, $ionicActionSheet, $cordovaImagePicker, $cordovaCamera, reqConfig, configUtil, fileUtil, $filter, $compile, $document, $ionicPlatform) {
        ...
        $scope.$on('$ionicView.beforeEnter', function () {
            ...
            DataEditService.getUnitParams().then(function (data) {
                $scope.pageData.unitParams = data.params;
            });

            DataEditService.getCostInfos(uid, cid, $scope.pageData.type).then(function (data) {
        ...
        $scope.$on("$ionicView.afterEnter", function () {
            ...
            $scope.pageData.fieldData.projectName = $scope.pageData.fieldData.project_id;
            ...
            DataDetailsService.getFileList($scope.pageData.fieldData.id).then(function(data) {
                $scope.pageData.fileList = data.fileList;
            });

            setTimeout(function(){
                var liTypeElem = $("ion-view[nav-view=active]").find("li[type]");
        ...
        $scope.delPic = function ($event) {
            var _me = $($event.target).parent("li");
            if(_me && _me.attr("id")){
        ...
        $scope.delOldPic = function ($event) {
            var _me = $($event.target).parent("li");
            if(_me && _me.attr("id")){
                DataEditService.delFile( _me.attr("id")).then(function(result) {
                    $ionicLoading.show({
                        template: '删除成功',
                        duration: reqConfig.loadingDuration
                    });
                    $.each($scope.pageData.fileList, function (i, imgObj) {
                        if(imgObj.id == _me.attr("id")){
                            $scope.pageData.fileList.splice(i, 1);
                            return false;
                        }
                    });
                    _me.remove();
